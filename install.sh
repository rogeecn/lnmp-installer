...
# tar zxf packages/cmake-3.15.0.tar.gz -C src/

# echo "unzip packages: libzip..."
# tar zxf packages/libzip-1.5.2.tar.gz -C src/

# echo "unzip packages: openresty..."
# tar zxf packages/openresty-1.15.8.1.tar.gz -C src/

# echo "unzip packages: php..."
# tar zxf packages/php-7.3.8.tar.gz -C src/
...
gmake && gmake install
cp packages/nginx.service /usr/lib/systemd/system/nginx.service
...
make && make install
cp packages/php.ini /usr/local/php/73/lib/php.ini
cp packages/www.conf /usr/local/php/73/etc/php-fpm.d/www.conf
cp packages/php-fpm.conf /usr/local/php/73/etc/php-fpm.conf
cp packages/php-fpm.service /usr/lib/systemd/system/php-fpm.service
systemctl daemon-reload
...
cp packages/server.cnf /etc/my.cnf.d/server.cnf
